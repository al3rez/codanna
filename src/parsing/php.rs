...
impl PhpParser {
    ...
    fn process_function(
        ...
        *counter += 1;
        ...
        let mut symbol = Symbol::new(id, name, SymbolKind::Function, file_id, self.node_to_range(node));
    ...
    fn process_method(
        ...
        *counter += 1;
        ...
        let mut symbol = Symbol::new(id, name, SymbolKind::Method, file_id, self.node_to_range(node));
        symbol.doc_comment = self.extract_doc_comment(&node, code).map(|s| s.into());
        Some(symbol)
    }
    ...
    fn process_class(
        ...
        *counter += 1;
        ...
        let mut symbol = Symbol::new(id, name, SymbolKind::Class, file_id, self.node_to_range(node));
        symbol.doc_comment = self.extract_doc_comment(&node, code).map(|s| s.into());
        Some(symbol)
    }
    ...
    fn process_interface(
        ...
        *counter += 1;
        ...
        let mut symbol = Symbol::new(id, name, SymbolKind::Interface, file_id, self.node_to_range(node));
        symbol.doc_comment = self.extract_doc_comment(&node, code).map(|s| s.into());
        Some(symbol)
    }
    ...
    fn process_trait(
        ...
        *counter += 1;
        ...
        let mut symbol = Symbol::new(id, name, SymbolKind::Trait, file_id, self.node_to_range(node));
        symbol.doc_comment = self.extract_doc_comment(&node, code).map(|s| s.into());
        Some(symbol)
    }
            ...
            if child.kind() == "property_element" {
                if let Some(name_node) = child.child_by_field_name("name") {
                    ...
                    *counter += 1;
                    ...
                    let mut symbol = Symbol::new(id, clean_name, SymbolKind::Field, file_id, self.node_to_range(node));
                    symbol.doc_comment = self.extract_doc_comment(&node, code).map(|s| s.into());
                    return Some(symbol);
                }
            }
            ...
            if child.kind() == "const_element" {
                if let Some(name_node) = child.child_by_field_name("name") {
                    ...
                    *counter += 1;
                    ...
                    let mut symbol = Symbol::new(id, name, SymbolKind::Constant, file_id, self.node_to_range(node));
                    symbol.doc_comment = self.extract_doc_comment(&node, code).map(|s| s.into());
                    return Some(symbol);
                }
            }
...
impl LanguageParser for PhpParser {
    fn parse(&mut self, code: &str, file_id: FileId, symbol_counter: &mut u32) -> Vec<Symbol> {
        ...
        self.extract_symbols_from_node(tree.root_node(), code, file_id, &mut symbols, symbol_counter);
        symbols
    }
        ...
        if let Some(prev) = node.prev_sibling() {
            if prev.kind() == "comment" {
                ...
                if comment_text.starts_with("/**") {
                    ...
                    // Single line comment
                    return Some(comment_text.strip_prefix("//").unwrap_or("").trim().to_string());
                }
            }
        }
        None
...
impl PhpParser {
    ...
    fn extract_imports_from_node(
        ...
        if matches!(node.kind(), "require_expression" | "require_once_expression" | 
